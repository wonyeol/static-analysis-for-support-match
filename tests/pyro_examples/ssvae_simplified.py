...
def guide(xs, ys=None):
    ...
    with pyro.plate("data"):
        ...
        shape = ys.shape[:-1] + (-1,)            
        hidden_z = softplus(encoder_z_fst(torch.cat([xs.expand(shape), ys.expand(shape)], -1))) 
...
class SSVAE(nn.Module):
    ...
    def setup_networks(self):
        ...
        self.encoder_y_fst = encoder_y_fst
        self.encoder_y_snd = encoder_y_snd

        self.encoder_z_fst = encoder_z_fst
        self.encoder_z_out1 = encoder_z_out1
        self.encoder_z_out2 = encoder_z_out2

        self.decoder_fst = decoder_fst
        self.decoder_snd = decoder_snd
        if self.use_cuda:
            self.cuda()
