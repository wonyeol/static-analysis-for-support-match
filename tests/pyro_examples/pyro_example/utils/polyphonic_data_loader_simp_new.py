"""
Source: /examples/pyro/dmm/utils/polyphonic_data_loader_simp_new.py
"""
    ...
    with open(file_loc, "rb") as f:
        ...
        for k, v in dset.items():
            ...
            for elt in sequences:
                assert(len(elt.shape) == 2 and elt.shape[1] == 88)
                tmp = torch.zeros(MAX_T, 88)
                tmp[:elt.shape[0], :] = elt
                seq_new.append(tmp)
            sequences = seq_new
            #=== wy
...
def get_mini_batch(mini_batch_indices, sequences, seq_lengths, cuda=False):
    ...
    #=== wy: force sequence_length be MAX_T during training, testing, validation.
    T_max = MAX_T
    #=== wy
    ...
    if debug:
        ...
        print("mini_batch_seq_lengths:\t type={}, shape={}".
              format(type(sorted_seq_lengths), sorted_seq_lengths.size()))
